  ...
  Future<bool> getWebsiteCSData()  async {
    ...
    if (response.statusCode == 200) {
      dom.Document document = parser.parse(response.body);
       ...
       ExtractedCSTitles = document.querySelectorAll(
          '.term-page--news-article--item--title').map((element) =>
          element.text.trim()).toList();
      ExtractedCSSubtitles = document.querySelectorAll(
          '.term-page--news-article--item--dek').map((element) =>
          element.text.trim()).toList();
      ExtractedCSDate = document.querySelectorAll(
          '.term-page--news-article--item--publication-date').map((element) =>
          element.text.trim()).toList();
     ...
     ExtractedCSImage = imageElements
          ...
          .toList();
            ...
            List<dom.Element> linkElement=document.querySelectorAll('.term-page--news-article--item--title a');
